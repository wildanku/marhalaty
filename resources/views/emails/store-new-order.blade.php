@extends('emails.layout')

@section('email_title', 'Pesanan Baru – ' . $order->order_number)
@section('header_icon', '🛍️')

@section('content')
    <h1 class="greeting">Pesanan baru masuk!</h1>
    <p class="lead-text">
        Toko <strong>{{ $store->name }}</strong> menerima pesanan baru yang sudah dibayar dan perlu
        diproses.
    </p>

    <table class="info-table" width="100%" cellpadding="0" cellspacing="0">
        @foreach ($order->items as $item)
            <tr>
                <td>{{ $item->name_snapshot }}{{ $item->variant_label_snapshot ? " ({$item->variant_label_snapshot})" : '' }} × {{ $item->quantity }}</td>
                <td>Rp {{ number_format((float) $item->subtotal, 0, ',', '.') }}</td>
            </tr>
        @endforeach
        <tr class="total-row">
            <td>Total</td>
            <td>Rp {{ number_format((float) $order->total, 0, ',', '.') }}</td>
        </tr>
    </table>

    <div class="cta-wrapper">
        <a href="{{ config('app.url') }}/my/stores/{{ $store->id }}/orders/{{ $order->id }}" class="cta-button">Proses Pesanan</a>
    </div>
@endsection
