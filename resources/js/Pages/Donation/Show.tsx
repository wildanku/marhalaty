    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-surface rounded-2xl w-full max-w-md p-6 shadow-xl">
        ...
        <div className="space-y-4">
          <div>
            <label className="block font-body text-sm font-medium text-on-surface-variant mb-1">
              Monthly Income (Rp)
            </label>
            <input
              type="number"
          ...
          <div className="flex gap-3 pt-4 border-t border-surface-container">
            <button
              onClick={onClose}
              className="flex-1 py-3 text-on-surface-variant font-label font-medium bg-surface-container hover:bg-surface-container-high rounded-full transition"
            >
              Cancel
            </button>
            <button
    ...
    <div className="bg-surface text-on-surface antialiased min-h-screen">
      ...
      <header className="bg-surface-container-low border-b border-surface-container-high py-4 px-6 sticky top-0 z-40">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <Link
            href="/maal"
            className="inline-flex items-center text-primary font-medium hover:underline text-sm font-label gap-1"
          >
      ...
      <main className="max-w-5xl mx-auto py-10 px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 flex flex-col gap-8">
          ...
          <div>
            <span
              className={`inline-block px-3 py-1 text-xs font-bold rounded-full mb-4 ${campaign.status === "active" ? "bg-primary/20 text-on-primary-container" : "bg-surface-container-high"}`}
            >
            ...
            <div className="mt-8 border-t border-surface-container pt-8">
              ...
              <div className="space-y-6">
                {campaign.updates.map((update) => (
                  <div
                    key={update.id}
                    className="relative pl-6 border-l-2 border-primary/30 pb-4 last:pb-0"
                  >
                    ...
                    <p className="font-body text-sm mt-2 text-on-surface-variant whitespace-pre-wrap">
                      {update.content}
                    </p>
                  </div>
                ))}
            ...
            {recentDonations.length > 0 ? (
              <div className="bg-surface-container-lowest rounded-xl border border-surface-container overflow-hidden">
                {recentDonations.map((donation, idx) => (
                  <div
                    key={donation.id}
                    className={`p-4 flex items-center justify-between ${idx !== recentDonations.length - 1 ? "border-b border-surface-container" : ""}`}
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-surface-container-high rounded-full overflow-hidden flex items-center justify-center text-outline">
                        {donation.user?.avatar_url ? (
                          <img
                            ...
                            alt={donation.user?.name || "Donor avatar"}
                      ...
                      <div>
                        <p
                          className={`font-headline text-sm font-bold ${donation.is_anonymous ? "text-on-surface-variant italic" : "text-on-surface"}`}
                        >
                    ...
                    <div className="font-body font-bold text-primary">
                      Rp {parseFloat(donation.amount).toLocaleString()}
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="w-full lg:w-[400px] shrink-0">
          <div className="bg-surface-container-lowest rounded-2xl shadow-[0px_10px_40px_rgba(80,100,71,0.06)] border border-surface-container-high p-6 sticky top-24">
            <div className="mb-6">
              ...
              <h3 className="font-headline font-bold text-3xl text-primary">
            ...
            <form onSubmit={submit} className="border-t border-surface-container pt-6">
              ...
              <div className="mb-4">
                ...
                {errors.donatable_type && (
                  <p className="text-error text-xs mt-1">{errors.donatable_type}</p>
                )}
              </div>
              ...
              <div className="mt-4 text-center">
                <button
                  ...
                  className="text-xs font-label text-primary font-semibold hover:underline flex items-center justify-center gap-1 w-full mt-2"
                >
