...
export default function Index({ auth, funds, campaigns }: MaalProps) {
  const totalCollected =
    funds.reduce((acc, curr) => acc + parseFloat(curr.collected_amount), 0) +
        ...
        <div className="flex-grow w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <section className="mb-16 relative rounded-xl overflow-hidden bg-surface-container-low p-8 md:p-16 flex flex-col md:flex-row items-center gap-8 shadow-[0px_10px_40px_rgba(80,100,71,0.08)]">
            ...
            <div className="relative z-10 flex-1">
              <span className="text-tertiary font-label font-bold tracking-widest uppercase text-xs mb-4 block">
                Baitul Maal Hub
              </span>
              <h1 className="text-4xl md:text-5xl font-headline font-extrabold text-on-surface mb-6 leading-tight tracking-tight">
                Empowering Legacy Through Generosity
              </h1>
              <p className="text-on-surface-variant font-body text-lg mb-8 max-w-xl leading-relaxed">
                Join hands with thousands of alumni to support education, infrastructure, and
                community welfare.
              </p>
              <div className="flex flex-wrap gap-4">
                <Link
                  href="#campaigns"
                  className="bg-tertiary text-on-tertiary px-8 py-4 rounded-full font-label font-bold shadow-sm hover:opacity-90 transition-all"
                >
                  Support Campaigns
                </Link>
              </div>
            ...
            <div className="relative z-10 flex-1 w-full max-w-md">
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                {funds.map((fund) => (
                  <div
                    key={fund.id}
                    className="bg-surface-container-lowest p-4 rounded-xl shadow-sm border border-surface-container-low flex flex-col"
                  >
                    ...
                    <div className="mt-auto pt-3">
                      <span className="inline-block py-1.5 w-full text-center text-primary font-medium text-xs border border-primary/20 hover:bg-primary/5 rounded-full transition-colors cursor-pointer">
                        Donate to {fund.name}
                      </span>
                    </div>
                  </div>
            ...
            <div className="flex justify-between items-end mb-8">
              <div>
                ...
                <p className="text-on-surface-variant font-body mt-2">
                  Projects requiring immediate attention and support.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {campaigns.map((campaign, idx) => {
                ...
                return (
                  <Link
                    key={campaign.id}
                    href={`/maal/campaigns/${campaign.slug}`}
                    className={`${isLarge ? "md:col-span-2 lg:col-span-2 flex-row" : "flex-col"} bg-surface-container-lowest rounded-xl p-6 md:p-8 flex gap-6 hover:shadow-[0px_10px_40px_rgba(80,100,71,0.08)] transition-all duration-300 border border-surface-container`}
                  >
                    {campaign.image_url && (
                      <img
                        src={campaign.image_url}
                        alt={campaign.title}
                        className={`${isLarge ? "w-full md:w-1/3" : "w-full h-40"} object-cover rounded-lg`}
                      />
                    )}
                    <div className="flex-1 flex flex-col justify-between">
                      <div>
                        <h3
                          className={`${isLarge ? "text-2xl" : "text-xl"} font-headline font-bold text-on-surface mb-2 leading-tight`}
                        >
                          {campaign.title}
                        </h3>
                      ...
                      <div>
                        <div className="flex justify-between font-label text-sm mb-2">
                          ...
                          <span className="text-on-surface-variant shrink-0 relative mr-[-4px]">
                        ...
                        <button className="w-full py-2 bg-primary text-on-primary rounded-full font-label text-sm font-semibold hover:bg-primary-container transition-colors">
                          Contribute
                        </button>
                      </div>
                    </div>
