...
export default function Index({ auth, events, currentScope }: PageProps<{ events: GontorEvent[], currentScope: string }>) {
    return (
        <div className="bg-surface text-on-surface antialiased min-h-screen flex flex-col md:flex-row">
            ...
            <nav className="hidden md:flex flex-col h-screen w-64 fixed left-0 top-0 py-8 gap-2 bg-[#f4f3f1] dark:bg-[#1a1c1a] border-r border-surface-container-high z-50">
                ...
                <div className="flex-1 overflow-y-auto px-2 space-y-1">
                    ...
                    <Link href="/events" className="flex items-center gap-3 bg-[#8da382] text-white rounded-lg mx-2 px-4 py-3 shadow-sm transition-all duration-200 font-body text-sm font-medium translate-x-1">
            ...
            <main className="flex-1 md:ml-64 pb-24 md:pb-0 min-h-screen">
                <div className="px-6 md:px-12 py-8 max-w-7xl mx-auto">
                    <h2 className="font-headline text-4xl md:text-5xl font-bold text-on-surface tracking-tight mb-2">Events & Gatherings</h2>
                    <p className="font-body text-lg text-on-surface-variant max-w-2xl">Discover upcoming alumni events, reconnect with your marhalah, and participate in global gatherings.</p>
                </div>
                ...
                <div className="px-6 md:px-12 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    <div className="lg:col-span-8 flex flex-col gap-6">
                        ...
                        {events.length === 0 ? (
                            <div className="p-8 text-center bg-surface-container-lowest rounded-xl shadow border border-surface-container">
                                ...
                                <p className="text-on-surface-variant font-body">No upcoming events match your criteria.</p>
                            </div>
                        ) : (
                            events.map(event => (
                                    ...
                                    <article className="bg-surface-container-lowest rounded-xl p-5 hover:bg-surface-container-low hover:shadow-[0px_10px_40px_rgba(80,100,71,0.08)] cursor-pointer transition-all group shadow-[0px_2px_10px_rgba(0,0,0,0.02)]">
                                        <div className="flex justify-between items-start mb-3">
                                            <span className="bg-surface-variant text-on-surface px-2.5 py-1 rounded-md text-xs font-semibold tracking-wide uppercase">
                                                {event.visibility_scope === 'global' || !event.visibility_scope ? 'Global' : `Marhalah ${event.visibility_scope}`}
                                            </span>
                                            <span className="bg-primary/10 text-primary px-2.5 py-1 rounded-md text-xs font-medium uppercase tracking-wider">
                                                {event.payment_type}
                                            </span>
                                        </div>
                                        <h3 className="font-headline text-xl font-bold text-on-surface mb-1 group-hover:text-primary transition-colors">{event.title}</h3>
                                        <p className="font-body text-sm text-on-surface-variant mb-4">{event.location}</p>
                    ...
                    <div className="lg:col-span-4 bg-surface-container-lowest rounded-2xl shadow-[0px_10px_40px_rgba(80,100,71,0.08)] p-8 text-center hidden lg:flex flex-col items-center justify-center min-h-[400px]">
                        <span className="material-symbols-outlined text-6xl text-primary/20 mb-4">touch_app</span>
                        <h3 className="font-headline font-bold text-xl text-on-surface mb-2">Select an event</h3>
                        <p className="text-on-surface-variant font-body text-sm max-w-[200px]">Click on any event from the list to view its full details and RSVP form.</p>
                    </div>
                </div>
            </main>
        </div>
